...
tf.flags.DEFINE_integer("num_epochs", 500000, "Number of training epochs")
tf.flags.DEFINE_integer("evaluate_every", 10, "Evaluate model on dev set after this many steps (default: 100)")
tf.flags.DEFINE_integer("checkpoint_every", 10, "Save model after this many steps (default: 100)")
tf.flags.DEFINE_integer("pools_size", 100, "The sampled set of a positive sample, which is bigger than 500")
...
tf.flags.DEFINE_integer("sampled_size", 100, " the real selected set from the The sampled pools")
...
print(("Loading data..."))
vocab = insurance_qa_data_helpers.build_vocab()  # '/train',/test1',/test2',/dev'
...
alist = insurance_qa_data_helpers.read_alist()  # [...]  ('insuranceQA/train')  A 全部的answer数据
raw = insurance_qa_data_helpers.read_raw()  # [...]  ('insuranceQA/train')     Q,A 正的数据
                ...
                for i in range(FLAGS.num_epochs):
                    ...
                    for g_epoch in range(FLAGS.g_epochs_num):  ## 1
                        ...
                        for _index, pair in enumerate(raw):
                            ...
                            neg_alist_index = [item for item in range(len(alist))]     ## [0,1,2,3,...,18540-1]
                            ...
                            pools = np.array(alist)[sampled_index]  # 随机选取alist中的99个语句,同时包含了自己的后一个match语句, 作为候选语句
                            ...
                            neg_index = np.random.choice(np.arange(len(pools)), size=FLAGS.gan_k, p=prob,
                                                         replace=False)  # 在100个例子中按照prob生成 FLAGS.gan_k个负例
                            subsamples = np.array(
